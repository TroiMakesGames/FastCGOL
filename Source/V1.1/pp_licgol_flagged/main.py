...
def updateWorld(world, active):
    ...
    hasCoordBeenChecked = [[False for _ in range(worldHeight)] for _ in range(wolrdWidth)]

    #go through each cell and check for rules
    for activeCell in active:
        x, y = activeCell
        if not hasCoordBeenChecked[x][y]:
            hasCoordBeenChecked[x][y] = True
            ...
            if nextWorld[x][y] != world[x][y]:
                for dx, dy in neighbours:
                    nx = (x + dx) % wolrdWidth
                    ny = (y + dy) % worldHeight
                    nextActive.append((nx, ny))
                nextActive.append((x, y))
