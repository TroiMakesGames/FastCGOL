class Grid_simulation 
{
    constructor(worldWidth, worldHeight) 
    {
        ...
        for (let i = 0; i < this.grid.length; i++) 
        {
            ...
            {this.activeCoords.push(i * this.worldHeight + j);}
        }

        //flag grid
        this.hasCoordBeenChecked = Array.from({ length: worldWidth }, () =>
            Array(worldHeight).fill(false)
        );
    ...
    Update() 
    {
        ...
        let newActiveCoords = [];
        //clear has coord ben checked flag grid
        this.hasCoordBeenChecked = Array.from({ length: this.worldWidth }, () =>
            Array(this.worldHeight).fill(false)
        );

        for (let n = 0; n < this.activeCoords.length; n++) 
        {
            const key = this.activeCoords[n];
            const i = Math.floor(key / this.worldHeight);
            const j = key % this.worldHeight;

            if (this.hasCoordBeenChecked[i][j] == false)
            {
                let nCount = 0;
                ...
                {
                    //add self
                    newActiveCoords.push(i * this.worldHeight + j);
                    ...
                    for (let n = 0; n < this.neighbours.length; n++) 
                    {
                        ...
                        newActiveCoords.push(nX * this.worldHeight + nY);
                    }
                }

                this.hasCoordBeenChecked[i][j] = true;
            }
        }
...
/* obj instantiation */
const grid = new Grid_simulation(250, 250);
let genCount = 0;
